...
Describe 'Remove-ZoomUserSchedulers' {
    Context 'When removing all user schedulers' {
        BeforeEach {
        ...
        It 'Should call API with correct endpoint' {
            Mock Invoke-ZoomRestMethod -ModuleName PSZoom {
                param($Uri, $Method)
                $Uri.ToString() | Should -Match 'users/testuser@example.com/schedulers'
                $Method | Should -Be 'DELETE'
                return $null
            }

            Remove-ZoomUserSchedulers -UserId 'testuser@example.com'
    ...
    Context 'Pipeline support' {
        ...
        It 'Should accept UserId from pipeline' {
            'user@example.com' | Remove-ZoomUserSchedulers
            Should -Invoke Invoke-ZoomRestMethod -ModuleName PSZoom -Times 1
        }

        It 'Should process multiple UserIds from pipeline' {
            @('user1@example.com', 'user2@example.com') | Remove-ZoomUserSchedulers
        ...
        It 'Should accept object with id property from pipeline' {
            ...
            $userObject | Remove-ZoomUserSchedulers
            Should -Invoke Invoke-ZoomRestMethod -ModuleName PSZoom -Times 1
        }
    ...
    Context 'Passthru parameter' {
        ...
        It 'Should return UserId array when Passthru is specified' {
            $result = Remove-ZoomUserSchedulers -UserId 'user@example.com' -Passthru
            $result | Should -Contain 'user@example.com'
        }

        It 'Should not return output when Passthru is not specified' {
            $result = Remove-ZoomUserSchedulers -UserId 'user@example.com'
            $result | Should -BeNullOrEmpty
        }

        It 'Should return correct UserId for each user when processing multiple users with Passthru' {
            $results = @('user1@example.com', 'user2@example.com') | Remove-ZoomUserSchedulers -Passthru
    ...
    Context 'Parameter aliases' {
        ...
        It 'Should accept Email alias for UserId' {
            { Remove-ZoomUserSchedulers -Email 'user@example.com' } | Should -Not -Throw
        }

        It 'Should accept EmailAddress alias for UserId' {
            { Remove-ZoomUserSchedulers -EmailAddress 'user@example.com' } | Should -Not -Throw
        }

        It 'Should accept Id alias for UserId' {
            { Remove-ZoomUserSchedulers -Id 'user123' } | Should -Not -Throw
        }

        It 'Should accept user_id alias for UserId' {
            { Remove-ZoomUserSchedulers -user_id 'user@example.com' } | Should -Not -Throw
        }
    }
    ...
    Context 'Multiple users processing' {
        ...
        It 'Should process each user separately' {
            Remove-ZoomUserSchedulers -UserId @('user1@example.com', 'user2@example.com', 'user3@example.com')
        ...
        It 'Should call correct endpoint for each user' {
            ...
            Mock Invoke-ZoomRestMethod -ModuleName PSZoom {
                ...
                if ($script:callCount -eq 1) {
                    $Uri.ToString() | Should -Match 'users/user1@example.com/schedulers'
                }
                if ($script:callCount -eq 2) {
                    $Uri.ToString() | Should -Match 'users/user2@example.com/schedulers'
                }
                return $null
            }

            Remove-ZoomUserSchedulers -UserId @('user1@example.com', 'user2@example.com')
        }
    }
    ...
    Context 'Error handling' {
        It 'Should propagate API errors' {
            ...
            { Remove-ZoomUserSchedulers -UserId 'nonexistent@example.com' -ErrorAction Stop } | Should -Throw
        }
        ...
        It 'Should handle network errors' {
            ...
            { Remove-ZoomUserSchedulers -UserId 'user@example.com' -ErrorAction Stop } | Should -Throw
        }
    }

    Context 'CmdletBinding attributes' {
        It 'Should not have SupportsShouldProcess' {
            $cmd = Get-Command Remove-ZoomUserSchedulers
