...
function New-ZoomMeeting {
  [CmdletBinding(DefaultParameterSetName="ScheduledMeeting")]
  param (
    [Parameter(
        Mandatory=$True, 
    ...
    [Parameter(
        Mandatory=$True, 
        ParameterSetName='RecurrenceByMonthWeek', ValueFromPipelineByPropertyName = $True
    )]
    [ValidateRange(1,90)]
  ...
  process {
    ...
    if (('ScheduledMeeting', 'RecurrenceByDay', 'RecurrenceByWeek', 'RecurrenceByMonthDay', 'RecurrenceByMonthWeek').Contains($PSCmdlet.ParameterSetName)) {
        $StartTime = get-date $StartTime -format yyyy-MM-ddTHH:mm:ssZ
